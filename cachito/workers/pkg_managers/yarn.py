...
from cachito.errors import FileAccessError, InvalidFileFormat, InvalidRepoStructure
...
def _get_deps(package_json, yarn_lock, file_deps_allowlist):
    ...
    :raise FileAccessError: if the lock file contains a dependency from an unsupported location
    """
    deps = []
    nexus_replacements = {}
    non_dev_deps = set()
    ...
    for dep_identifier, dep_data in yarn_lock.items():
        ...
        if package.url:
            ...
            raise FileAccessError(f"The dependency {package.name}@{package.version} has no source")
...
def _convert_to_nexus_hosted(dep_name, dep_source, dep_info):
    ...
    :raise FileAccessError: if the dependency is from an unsupported location
...
def _get_package_and_deps(package_json_path, yarn_lock_path):
    ...
    try:
        ...
        raise InvalidFileFormat("The package.json file is missing required data (name, version)")
...
def resolve_yarn(app_source_path, request, skip_deps=None):
    ...
    :rtype: dict
    :raises InvalidFileFormat: if fetching the dependencies fails (missing data in files)
    :raises InvalidRepoStructure: if required files are missing
    """
