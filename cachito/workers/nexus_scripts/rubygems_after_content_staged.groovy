/*
This script configures Nexus so that the Rubygems proxy repository for the Cachito request is blocked from getting additional
content.

In addition, a temporary user for the request is created and is given permission to access this Rubygems proxy repository.
...
void blockOutboundConnections(String repositoryName) {
    logger.info("Blocking outbound connections from the Rubygems proxy repository ${repositoryName}")
...
blockOutboundConnections(request.rubygems_repository_name)
...
List<String> privileges = ["nx-repository-view-rubygems-${request.rubygems_repository_name}-read".toString()]
// Create a role that has read access on the new repository. This will allow a user with this role to utilize the
// the Rubygems proxy for this Cachito request.
createRole(roleName, "Read access for ${request.rubygems_repository_name}".toString(), privileges)
