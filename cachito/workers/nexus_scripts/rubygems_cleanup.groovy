/*
This script deletes the RubyGems hosted repository, user, and role for the Cachito request.
...
['rubygems_repository_name', 'username'].each { param ->
...
logger.info("Deleting the repository ${request.rubygems_repository_name}")
if (repository.repositoryManager.exists(request.rubygems_repository_name)) {
    repository.repositoryManager.delete(request.rubygems_repository_name)
} else {
    logger.warn("The repository ${request.rubygems_repository_name} was not found")
}

return "The RubyGems repository ${request.rubygems_repository_name} and the user and role ${request.username} are removed"
