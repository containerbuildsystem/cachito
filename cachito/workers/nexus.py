...
def create_or_update_script(script_name, script_path):
    ...
    :raise NexusError: if Nexus script does not exist/cannot be modified
    """
    config = get_worker_config()
    ...
    if rv_get.status_code == 404:
        ...
        raise NexusError(f"Failed to determine if the Nexus script {script_name} exists")
    ...
    if not rv_script.ok:
        ...
        raise NexusError(f"Failed to create/update the Nexus script {script_name}")
...
def search_components(in_nexus_hoster=True, **query_params):
    ...
    :rtype: list<dict>
    :raise NexusError: if the search fails
    """
    config = get_worker_config()
    if in_nexus_hoster:
...
def upload_asset_only_component(repo_name, repo_type, component_path, to_nexus_hoster=True):
    ...
    :raise NetworkError, NexusError: if the upload fails
...
def upload_component(params, payload, to_nexus_hoster, additional_data=None):
    ...
    if not rv.ok:
        ...
        raise NexusError("Failed to upload a component to Nexus")
