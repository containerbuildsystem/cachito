...
from cachito.errors import GoModError
...
def fetch_gomod_source(request_id, dep_replacements=None, package_configs=None):
    ...
    :raises GoModError: if the dependencies could not be retrieved
    """
    ...
    if len(subpaths) > 1 and dep_replacements:
        raise GoModError("Dependency replacements are only supported for a single go module path.")
