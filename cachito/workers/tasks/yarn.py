...
from cachito.errors import InvalidFileFormat, InvalidRepoStructure, ValidationError
...
def _verify_yarn_files(bundle_dir: RequestBundleDir, subpaths: List[str]):
    ...
    :raises InvalidRepoStructure: if the repository is missing the required files or contains
...
def fetch_yarn_source(request_id: int, package_configs: List[dict] = None):
    ...
    :raise InvalidRepoStructure, InvalidFileFormat: if the task fails
    """
    ...
    for i, subpath in enumerate(subpaths):
        ...
        except (InvalidFileFormat, InvalidRepoStructure):
