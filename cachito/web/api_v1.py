...
class RequestsArgs(pydantic.BaseModel):
    ...
    error_origin: Union[RequestErrorOrigin, str, None]
...
def get_requests():
    ...
    if isinstance(args.error_origin, str):
        try:
            args.error_origin = RequestErrorOrigin[args.error_origin]
        except KeyError:
            raise KeyError(f"Following error_origin value is not valid: {args.error_origin}")
...
def get_request_metrics_summary():
    ...
    client_errors = (
        db.session.query(RequestError)
        .filter(RequestError.origin == RequestErrorOrigin.client)
        .count()
    )
    server_errors = (
        db.session.query(RequestError)
        .filter(RequestError.origin == RequestErrorOrigin.server)
        .count()
    )
